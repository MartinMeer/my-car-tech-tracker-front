...
export default function InMaintenance() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load cars and maintenance entries from DataService
      const [carsData, maintenanceData] = await Promise.all([
        DataService.getCars(),
  ...
  const confirmReturnToService = async () => {
    if (!entryToReturn) return

    try {
      // Remove from maintenance list via DataService
      await DataService.removeMaintenanceEntry(entryToReturn.carId)
      ...
      console.error('Error returning car to ready status:', error)
      alert('Ошибка при возвращении автомобиля в работу')
    }
  }
