...
export default function MaintenancePlanning() {
  ...
  // Mock car data
  const cars = [
    { id: '1', name: 'Транспорт #1', brand: 'Toyota', model: 'Camry', currentMileage: 85000, lastServiceDate: '2024-07-15' },
    { id: '2', name: 'Грузовик #2', brand: 'Mercedes', model: 'Sprinter', currentMileage: 120000, lastServiceDate: '2024-06-20' }
  ]
  ...
  useEffect(() => {
    if (selectedCarId) {
      ...
      if (selectedCar) {
        ...
        setPlannedMileage(selectedCar.currentMileage.toString())
      }
    }
  }, [selectedCarId, userAlerts])
                ...
                <SelectContent>
                  {cars.map((car) => (
                    <SelectItem key={car.id} value={car.id}>
                      {car.name} - {car.brand} {car.model} ({car.currentMileage.toLocaleString()} км)
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
